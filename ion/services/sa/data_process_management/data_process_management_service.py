...
class DataProcessManagementService(BaseDataProcessManagementService):
    ...
    def create_data_process(self, data_process={}, in_subscription_id='', out_data_product_id=''):
        """
        @param      data_process          dict defining the general resource and specific transform
        @param      in_subscription_id    ordered list of data product IDs matching inputs
        @param      out_data_product_id   ID of data product to publish process output to
        @retval     data_process_id       ID of the created data process object
        """
        log.debug("DataProcessManagementService:create_data_process: " + str(data_process))

        # Validation
        if not "name"
        if not data_process[atbd_reference]:
            raise BadRequest("Invalid ATBD reference " + str(data_process[atbd_reference]))
        ...
        transform_id = self.clients.transform_management_service.create_transform(transform_object)
        ...
        # TODO: Flesh out more details of transform mgmt service's schedule and bind methods
    ...
    def update_data_process(self, data_process={}):
        ...
        """
        log.debug("DataProcessManagementService:update_data_process: " + str(data_process))
        ...
        for x in transform_ids:
            ...
            transform_obj.in_subscription_id = in_subscription_id
            transform_obj.out_data_product_id = out_data_product_id
    ...
    def read_data_process(self, data_process_id=''):
        ...
        if not data_process_obj:
            raise NotFound("Data Process does not exist " + str(data_process_id))
    ...
    def delete_data_process(self, data_process_id=''):
        ...
        @retval     {success: True or False}
        """
        log.debug("DataProcessManagementService:delete_data_process: " + str(data_process_id))
