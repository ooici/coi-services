...
class TestMarineFacilityManagementServiceIntegration(IonIntegrationTestCase):
    ...
    def setUp(self):
        ...
        self.MFMS = MarineFacilityManagementServiceClient(node=self.container.node)
    ...
    def test_find_subordinate(self):
        ...
        ret = self.MFMS.find_subordinate_entity(stuff.marine_facility_id, [RT.LogicalInstrument])
        ids = idify(ret)
        self.assertIn(RT.LogicalInstrument, ids)
        self.assertIn(stuff.logical_instrument_id, ids[RT.LogicalInstrument])

        #partial traversal, only down to platform
        ret = self.MFMS.find_subordinate_entity(stuff.marine_facility_id, [RT.Site, RT.LogicalPlatform])
        ids = idify(ret)
    ...
    def _make_associations(self):
        ...
        self.RR.create_association(site2_id, PRED.hasPlatform, logical_platform_id)
        

        ret = DotDict()
        ret.marine_facility_id     = marine_facility_id
        ret.site_id                = site_id
        ret.site2_id               = site2_id
        ret.logical_platform_id    = logical_platform_id
        ret.logical_platform2_id   = logical_platform2_id
    ...
    def test_find_marine_facility_org(self):
        marine_facility_obj = IonObject(RT.MarineFacility,
                                        name='TestFacility',
                                        description='some new mf')
        marine_facility_id = self.MFMS.create_marine_facility(marine_facility_obj)
        org_id = self.MFMS.find_marine_facility_org(marine_facility_id)
        print("org_id=<" + org_id + ">")
        ...
        site_id = self.MFMS.create_site(site_obj, marine_facility_id)
        ...
        self.MFMS.assign_resource_to_marine_facility(resource_id=site_id, marine_facility_id=marine_facility_id)
        ...
        logical_platform_id = self.MFMS.create_logical_platform(logical_platform_obj, site_id)
        ...
        self.MFMS.assign_resource_to_marine_facility(resource_id=logical_platform_id, marine_facility_id=marine_facility_id)
        ...
        logical_instrument_id = self.MFMS.create_logical_instrument(logical_instrument_obj, logical_platform_id)
        ...
        self.MFMS.assign_resource_to_marine_facility(resource_id=logical_instrument_id, marine_facility_id=marine_facility_id)
        ...
        self.MFMS.unassign_resource_from_marine_facility(logical_instrument_id, marine_facility_id)
        # verify that LogicalInstrument is linked to Org
        ...
        self.MFMS.delete_logical_instrument(logical_instrument_id)
        ...
        self.MFMS.unassign_resource_from_marine_facility(logical_platform_id, marine_facility_id)
        # verify that LogicalPlatform is linked to Org
        assocs,_ = self.RR.find_objects(org_id, PRED.hasResource, RT.LogicalPlatform, id_only=True )
        self.assertEqual(len(assocs), 0)

        # remove the LogicalPlatform
        self.MFMS.delete_logical_platform(logical_platform_id)
        ...
        self.MFMS.unassign_resource_from_marine_facility(site_id, marine_facility_id)
        # verify that Site is linked to Org
        assocs,_ = self.RR.find_objects(org_id, PRED.hasResource, RT.Site, id_only=True )
        self.assertEqual(len(assocs), 0)

        # remove the Site
        self.MFMS.delete_site(site_id)
