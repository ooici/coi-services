...
# Get a visualization image for the provenance of a specific data product parameter
...
def get_parameter_provenance_visualization_image():
    ...
    # Create client to interface with the viz service
    data_product_cli = DataProductManagementServiceClient()   #, process=service_gateway_instance)

    if 'serviceRequest' not in json_params or 'params' not in json_params['serviceRequest']:
        raise BadRequest("The JSON request is not properly formed")
    params = json_params['serviceRequest']['params']
    if 'data_product_id' not in params or 'parameter_name' not in params:
        raise BadRequest("The JSON request is not properly formed")
...
def upload_status(fuc_id):
    # @TODO: This is not an upload (to the system) and not restricted - REMOVE
    try:
        object_store = Container.instance.object_store
