...
class IngestionManagementService(BaseIngestionManagementService):
    ...
    def launch_worker(self, queue_name):
        config = DotDict()
        config.process.datastore_name = self.CFG.get_safe('bootstrap.processes.ingestion.datastore_name', 'datasets')
        config.process.queue_name = queue_name
        config.process.buffer_limit = self.CFG.get_safe('bootstrap.processes.ingestion.buffer_limit', 10)
        config.process.time_limit = self.CFG.get_safe('bootstrap.processes.ingestion.time_limit', 10)
