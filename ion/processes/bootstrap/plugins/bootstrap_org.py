...
class BootstrapOrg(BootstrapPlugin):
    ...
    def on_initial_bootstrap(self, process, config, **kwargs):
        org_ms_client = OrgManagementServiceProcessClient(process=self)
        ex_ms_client = ExchangeManagementServiceProcessClient(process=self)

        # Create root Org: ION
        root_orgname = config.system.root_org
        org = Org(name=root_orgname, description="ION Root Org")
        self.org_id = org_ms_client.create_org(org, headers={'ion-actor-id': self.system_actor_id})
        ...
        org_ms_client.grant_role(self.org_id, system_actor._id,ION_MANAGER, headers={'ion-actor-id': self.system_actor_id} )

        # Make the ION system agent a manager for the ION Org
        org_ms_client.grant_role(self.org_id, system_actor._id,MANAGER_ROLE, headers={'ion-actor-id': self.system_actor_id} )

        # Create root ExchangeSpace
        xs = ExchangeSpace(name=ION_ROOT_XS, description="ION service XS")
        self.xs_id = ex_ms_client.create_exchange_space(xs, self.org_id, headers={'ion-actor-id': self.system_actor_id})
        ...
        process.container.resource_registry.resource_registry.create(neg_def)
