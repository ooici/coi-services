...
class DispatcherVisualizationException(IonException):
    """
...
class DispatcherVisualization(BaseReplayProcess):

    process_type = 'standalone'

    def __init__(self, *args, **kwargs):

        super(DispatcherVisualization,self).__init__(*args,**kwargs)
        self.lock = RLock()
    ...
    def on_start(self):
        ...
        if not (self.stream_id and hasattr(self, 'output')):
            raise RuntimeError('The dispatcher agent requires an output stream publisher named output. Invalid configuration!')
    ...
    def _parse_results(self, results):
        '''
        @brief Switch-case logic for what packet types dispatcher can handle and how to handle
        ...
        for result in results:
            ...
            log.warn('Unknown packet type in dispatcher.')
    ...
    def merge_granule(definition, granule1, granule2):
        ...
        fields1 = DispatcherVisualization._list_data(definition, granule1)
        fields2 = DispatcherVisualization._list_data(definition, granule2)
    ...
    def _merge(self, msgs):
        ...
        for i in xrange(count):
            if i==0:
                ...
                res = DispatcherVisualization.merge_granule(definition=self.definition, granule1=granule, granule2=None)
                granule = res['granule']
                file_pair = res['files']
                ...
                res = DispatcherVisualization.merge_granule(definition=self.definition, granule1=granule, granule2=msgs[i]['granule'])
