...
class PlatformDriver(object):
    ...
    def __init__(self, nnode, evt_recv):
        """
        Creates a PlatformDriver instance.

        @param nnode     Root NNode defining the platform network rooted at
                         this platform.
        @param evt_recv  Listener of events generated by this driver
        """
        assert nnode, "nnode must be given"
        assert evt_recv, "evt_recv parameter must be given"

        self._nnode = nnode
        self._send_event = evt_recv

        self._platform_id = self._nnode.platform_id
        if self._nnode.parent:
            self._parent_platform_id = self._nnode.parent.platform_id
        else:
            self._parent_platform_id = None

        self._platform_attributes = \
            dict((a.attr_id, a.defn) for a in self._nnode.attrs.itervalues())
    ...
    def _get_platform_attributes(self):
        ...
        construction time (from nnode parameter).
        """
